...
/** Turn logging on/off at runtime (the engine calls this with `config.debug`). */
export function configureLog(opts: { enabled?: boolean }) {
  if (typeof opts.enabled === 'boolean') {
    active = opts.enabled
    if (active && enabled.size === 0) enabled = new Set(ALL)
  }
}
...
export const log = {
  ...
  warn:  (...args: unknown[]) => active && console.warn('[warn]', ...args),
}
