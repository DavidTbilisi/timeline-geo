...
import type { EventImage, EventDetail } from '../../types/detail'
import { useTimelineConfig } from '../../config/inject'
import { resolveAsset } from '../../theme/assetUrl'
...
const images = computed<EventImage[]>(() => props.detail.images ?? [])
const current = ref(0)
...
function buildSrc(file: string) {
  return resolveAsset(config, config.assets.detailImageBase + encodeURIComponent(file))
}
