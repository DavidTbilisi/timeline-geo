...
import { useLocalized, localizeEraTokens } from '../../i18n'
import { formatDateRange } from '../../layout'
...
// then prefix with the configured asset base so it works on subpaths.
const imageUrl = computed(() => {
  if (!props.event.image) return ''
  const encoded = props.event.image.split('/').map(p => encodeURIComponent(p)).join('/')
  return resolveAsset(config, encoded)
})
