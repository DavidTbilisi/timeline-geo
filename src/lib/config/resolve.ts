...
export function resolveConfig(config: TimelineConfig): ResolvedTimelineConfig {
  ...
  return {
    ...
    routes: {
      ...
      periodPrefix: config.routes?.periodPrefix ?? '/period',
      eventPrefix: config.routes?.eventPrefix ?? '/event',
      catchAll: config.routes?.catchAll ?? true,
    },
    loaders: config.loaders,
    plugins: { detailTabs: config.plugins?.detailTabs, landingPanels: config.plugins?.landingPanels ?? [] },
    assets: {
      baseUrl: config.assets?.baseUrl ?? '/',
      paperBg: config.assets?.paperBg,
      gridLines: config.assets?.gridLines,
      detailImageBase: config.assets?.detailImageBase ?? 'media/images/original/',
    },
